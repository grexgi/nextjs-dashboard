...
app.get('/sensors/:cropId/:date', async (req, res) => {
    ...
    try {
        const [rows] = pool.query('SELECT * FROM sensing_data WHERE crop_id = ? AND date = ?', [cropId, date]);
        if (rows.length === 0) {
            return res.status(404).json({ message: 'No sensor data found for this crop and date.' });
        }
